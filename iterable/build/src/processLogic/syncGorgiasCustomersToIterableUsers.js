const customerToUserPayload = (customer, listId) => {
    try {
        if (!customer.email) {
...
export const syncGorgiasCustomersToIterableUsers = async (gorgias, iterable, config) => {
    try {
        ...
        console.error(`-------------- Starting to sync gorgias customers to iterable users --------------`);
        ...
        for await (const customer of customersGenerator) {
            ...
            try {
                ...
                console.error(`❌ There was an error in creating IterableClient user based on GorgiasClient customer ${customer.id}: ${JSON.stringify(error.response?.data) || error}, ${JSON.stringify(userPayload)}`);
            }
        }
        console.error(`-------------- Completed syncing gorgias customers to iterable users --------------`);
    }
    catch (error) {
        console.error(`Unexpected error: ${error}`);
        console.error(`-------------- Stopped syncing gorgias customers to iterable users --------------`);
    }
};
