...
/// <summary>
/// The points from which each of the two order queries resume, stored in tenant metadata between
/// runs. <see cref="NewOrders"/> is fetched oldest-first, so it advances per order;
/// <see cref="UpdatedOrders"/> is the minimum update across the whole result set, so it is
/// only known once the last page is read.
///
...
public sealed class Cursors(DateTime newOrders, DateTime updatedOrders)
{
    ...
    /// <summary>
    /// The cursors as Pandium stores them. Only these two keys are written, so the shallow
...
/// <summary>
/// The cron flow: ShipBob orders to the Gorgias customer sidebar. Keeps each Gorgias
/// customer's <c>data.pandium.shipbob_orders</c> in sync with their recent ShipBob orders,
/// resuming from the cursors in tenant metadata.
///
/// Pandium bounds a run at roughly ten minutes, so a large backlog will not finish in one
/// pass. The sync keeps a single <see cref="Cursors"/> current as each order is processed
/// and stops on the deadline token <c>Program.cs</c> sets; whatever the cursors had reached
/// is what the run writes back, and the next run picks up there.
///
/// The instance takes <see cref="IOrders"/> and <see cref="IHelpdesk"/> so the tests can
/// drive it with in-memory doubles; <see cref="RunAsync"/> wires up the real clients.
/// </summary>
public sealed class CronFlow(IOrders shipbob, IHelpdesk gorgias, ILogger<CronFlow> logger)
{
    ...
    /// <summary>
    /// Read the cursors out of tenant metadata, run the sync, and hand back the cursors to
        ...
        Pandium pandium, ILoggerFactory loggerFactory, CancellationToken token)
    {
        ...
        // Until the first run has written a cursor, the start date comes from the
        // order_start_date config the tenant filled in on the connection settings form.
    ...
    /// <summary>
    /// The orders of a paginated ShipBob query, until it answers with an empty page. Lazy
    /// on purpose: a page is fetched only once the previous one has been processed and the
    /// cursor has moved with it, so a run cut short has an honest cursor to write.
    /// </summary>
    ...
    private sealed class CustomerRecord
    {
        ...
        /// <summary>
        /// Merge the entry into the order list: replace the order with the same id, or add
        /// it and trim the list back to the most recent <see cref="MaxOrdersToSync"/>.
        /// </summary>
        public void AddOrder(JsonObject entry, bool newestFirst)
        {
            ...
            if (existing >= 0)
            {
                _orders[existing] = entry;
                return;
            }

            // Ids ascend with time, so the most recent orders are the highest ids.
            var kept = _orders
                .Append(entry)
                .OrderBy(order => order.Field("id").AsNumber() ?? 0)
                .TakeLast(MaxOrdersToSync);
            _orders = newestFirst ? [.. kept.Reverse()] : [.. kept];
        }

        /// <summary>
        /// The record as Gorgias receives it. Everything under <c>data</c> that this
        /// integration does not own is copied across untouched, so other tools' keys on the
        /// customer survive the write.
        /// </summary>
        public JsonObject Payload()
        {
            var pandium = Copy(_stored.Field("data").Field("pandium").AsMap(), except: "shipbob_orders");
            pandium["shipbob_orders"] = new JsonArray([.. _orders.Select(order => order?.DeepClone())]);

            var data = Copy(_stored.Field("data").AsMap(), except: "pandium");
            data["pandium"] = pandium;

            var payload = Copy(_stored, except: "data");
            if (Id is { } id)
            {
                payload["id"] = id;
            }
        ...
        /// <summary>
        /// A deep copy of <paramref name="source"/> minus one key. A <see cref="JsonNode"/>
        /// belongs to a single parent, so anything re-parented has to be cloned.
        /// </summary>
        private static JsonObject Copy(JsonObject? source, string except)
        {
