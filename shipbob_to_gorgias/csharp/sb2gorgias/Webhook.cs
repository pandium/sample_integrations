...
/// <summary>
/// The webhook flow: a Gorgias ticket for every ShipBob order webhook.
///
/// There is no web server here. Pandium receives each delivery, verifies its signature,
/// writes the body to disk, and starts a run in <c>webhook</c> mode. Deliveries are
/// debounced per tenant, so a run carries N of them and the flow loops over every one.
///
/// Creating a ticket is not idempotent and a delivery can repeat, so each one is deduped on
/// <c>shipment_id:status</c> against a <c>processed_events</c> map kept in tenant metadata
/// and pruned to a 30-minute window. Keying on the status as well as the shipment lets a
/// redelivery be dropped while the shipment's genuine <em>next</em> status still opens a
/// ticket.
///
/// The instance takes <see cref="IHelpdesk"/> so the tests can drive it with an in-memory
/// double; <see cref="RunAsync"/> wires up the real client.
/// </summary>
public sealed class WebhookFlow(IHelpdesk gorgias, ILogger<WebhookFlow> logger)
{
    ...
    /// <summary>
    /// Read <c>processed_events</c> out of tenant metadata, ticket this run's deliveries,
    /// and hand the pruned and extended map back to store.
    /// </summary>
        ...
        Pandium pandium, ILoggerFactory loggerFactory, CancellationToken token)
    {
        ...
        // Only this key is written, so the shallow merge leaves the cron flow's cursors
        // untouched.
        var events = new JsonObject();
        foreach (var (eventKey, ticketedAt) in processed)
        {
            events[eventKey] = ticketedAt;
        }

        return new JsonObject { ["processed_events"] = events };
    }
    ...
    /// <summary>
    /// The <c>POST /tickets</c> payload for a shipment webhook of any status. Only the parts
    /// ShipBob sent for this status make it into the body: an OnHold shipment has no
    /// tracking, a Delivered one has no status details.
    /// </summary>
    public static JsonObject BuildTicket(Shipment shipment, long customerId)
    {
        ...
        // Gorgias takes the customer twice: as the ticket's owner and as the sender of its
        // first message. A JsonNode has one parent, hence a fresh object each time.
