...
/// <summary>
/// How a Gorgias customer is identified: by recipient email when there is one, otherwise by
/// a synthetic <c>external_id</c> built from the recipient's name and address. A closed
/// hierarchy, so there is always exactly one key and the lookup and the created record
...
public abstract record CustomerKey
{
    ...
    /// <summary>
    /// Both flows key on this, so a webhook ticket lands on the record that carries the
...
/// <summary>
/// The real Gorgias. Auth is OAuth2 through Pandium's <c>gorgias-oauth</c> connector:
/// Pandium runs the authorization flow when the tenant connects and refreshes the token as
/// needed, so this client reads whichever access token is current from
/// <c>PAN_SEC_GORGIAS_OAUTH_ACCESS_TOKEN</c> and holds no client secret and no refresh
/// logic.
/// </summary>
public sealed class GorgiasClient : IHelpdesk, IDisposable
{
    ...
    public GorgiasClient(Pandium pandium, ILogger<GorgiasClient> logger)
    {
        ...
        // The connector also reports the token type; read it rather than assume Bearer.
