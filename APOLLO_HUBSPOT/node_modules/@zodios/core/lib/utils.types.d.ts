 ...
 */
type Split<Str, Sep extends string, Acc extends string[] = []> = Str extends "" ? Acc : Str extends `${infer T}${Sep}${infer U}` ? Split<U, Sep, [...Acc, T]> : [...Acc, Str];
type ConcatSplits<Parts extends string[], Seps extends string[], Acc extends string[] = []> = Parts extends [infer First extends string, ...infer Rest extends string[]] ? ConcatSplits<Rest, Seps, [...Acc, ...SplitMany<First, Seps>]> : Acc;
/**
 ...
 */
type SplitMany<Str extends string, Sep extends string[], Acc extends string[] = []> = Sep extends [
    infer FirstSep extends string,
    ...infer RestSep extends string[]
] ? ConcatSplits<Split<Str, FirstSep>, RestSep> : [Str, ...Acc];
type PathSeparator = ["/", "?", "&", "#", "=", "(", ")", "[", "]", "%"];
type FilterParams<Params, Acc extends string[] = []> = Params extends [
    infer First,
    ...infer Rest
] ? First extends `${string}:${infer Param}` ? FilterParams<Rest, [...Acc, ...Split<Param, ":">]> : FilterParams<Rest, Acc> : Acc;
/**
 ...
 * ```
 */
type ApiPathToParams<Path extends string> = FilterParams<SplitMany<Path, PathSeparator>>;
/**
 * get all parameters from an API path
 * @param Path - API path
 * @details - this is using tail recursion type optimization from typescript 4.5
 */
type PathParamNames<Path> = Path extends string ? ApiPathToParams<Path>[number] : never;
/**
...
export { ApiPathToParams, Assert, DeepReadonlyObject, DefinedArray, FilterArrayByKey, FilterArrayByValue, IfEquals, MapSchemaParameters, MaybeReadonly, Merge, MergeUnion, Narrow, NeverIfEmpty, OptionalChildProps, OptionalProps, PathParamNames, PickDefined, PickRequired, ReadonlyDeep, RequiredKeys, RequiredProps, SetPropsOptionalIfChildrenAreOptional, Simplify, Split, SplitMany, TupleFlat, UndefinedIfEmpty, UndefinedIfNever, UndefinedToOptional, UnionToIntersection, UnionToTuple };
