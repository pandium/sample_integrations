...
fn run(mode: &str, pandium: &Pandium) -> Value {
    eprintln!("------------------------CONFIG------------------------");
    eprintln!("{}", pandium.config_repr());

    eprintln!("------------------------SECRET------------------------");
    eprintln!("{}", pandium.secrets_repr());

    eprintln!("------------------------CONTEXT------------------------");
    eprintln!("{}", pandium.context_repr());

    match mode {
        // Init mode: populate tenant metadata with the dynamic config values
        // needed for the customer-facing config form. In the real world, these
        // values would be derived from an api call.
        "init" => json!({
        ...
        // Normal mode: log the previous normal run's random number and store a fresh random
        // number as metadata.
        _ => {
