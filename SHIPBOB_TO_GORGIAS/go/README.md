...
Go 1.27, standard library only — no third-party dependencies at all. No framework, no web
server.
...
├── dotenv.go          loads a local .env for development; no-op if there isn't one
...
**The run-limit deadline** is a `context.WithTimeout` in `cronRun`, checked cooperatively —
`runCron`'s paging loops test `ctx.Err()` between orders and between pages, and stop cleanly
rather than erroring out when it fires. Stopping there and returning the cursor normally is
still success: the partial progress merges into metadata and the next run resumes from it.
The same `ctx` is threaded down into every HTTP call (`http.NewRequestWithContext`), so an
in-flight request is torn down at the deadline too instead of finishing on its own:

```go
ctx, cancel := context.WithTimeout(context.Background(), alarmDuration)
defer cancel()
return runCron(ctx, pandium, cronDeps{ShipBob: shipbob, Gorgias: gorgias, Now: time.Now()})
```

Because the loop only ever runs on the one goroutine that's checking `ctx.Err()`, the cursor
(`cursorState` in `cron.go`) is a plain struct with no locking — there's no concurrent
goroutine touching it the way a `time.AfterFunc`-based timer would.

**No monkey-patching, so the client interfaces exist from the start.** Go can't reassign a
struct's methods at runtime for a test double. `cron.go` and
...
once, after the loop fully exhausts. A run cut short — by the deadline passing or by a fetch error —
...
19 tests covering both flows end to end — including the deadline flush, a fetch failure, and
...
- A ShipBob **order** stays `map[string]any` (decoded JSON) in `cron.go` — it's mostly passed
  straight through to the Gorgias sidebar unread, so `deepGet` in `lib.go` does a dotted-path
  lookup (e.g. `deepGet(order, "recipient.address.city", "")`) rather than modelling every
  field. A ShipBob **shipment webhook event**, by contrast, is `shipmentEvent` in `webhook.go`
  — a typed struct — since every field on it drives an actual decision (a ticket's body, its
  tags, its dedupe key), unlike an order's fields, which the code itself never inspects.
  `recipient`/`address` in `gorgias.go` are the one piece both share, so `customerKey` and
  `newCustomerPayload` stay single-sourced regardless of which side calls them.
