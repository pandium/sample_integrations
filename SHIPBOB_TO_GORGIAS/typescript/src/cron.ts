...
export function clamp(value: unknown, now: Date): Date {
...
export function upsert(orders: any[], orderPayload: any, newestFirst: boolean): any[] {
...
export interface CronDeps {
    ...
    gorgias?: GorgiasClient
    now?: Date
    armWatchdog?: (onTimeout: () => void) => { cancel: () => void }
    exit?: (code: number) => never
}

function defaultArmWatchdog(onTimeout: () => void): { cancel: () => void } {
    const timer = setTimeout(onTimeout, ALARM_MS)
    timer.unref?.() // don't hold the process open past a normal, faster completion
    return { cancel: () => clearTimeout(timer) }
}

export async function run(pandium: Pandium, deps: CronDeps = {}): Promise<Record<string, string>> {
