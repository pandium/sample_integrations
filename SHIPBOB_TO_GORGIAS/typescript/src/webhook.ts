...
export function prune(processed: Record<string, string>, now: Date): Record<string, string> {
...
export function shipmentId(event: any): string {
...
function statusDetails(event: any): string {
    const details = deepGet(event, 'status_details', []) || []
    return details
        .filter((d: any) => d)
        .map((d: any) => d.description || d.name || '')
        .join('; ')
}

/** One line per product on the shipment: `4 x 16 oz. Shampoo (PIN-100)`. */
function items(event: any): string {
    const lines: string[] = []
    for (const product of deepGet<any[]>(event, 'products', []) || []) {
        const quantity = (product.inventory_items ?? []).reduce(
 ...
 */
export function buildTicket(event: any, customerRef: { id: number }): any {
 ...
 */
export async function resolveCustomer(gorgias: GorgiasClient, event: any): Promise<{ id: number }> {
...
export async function run(
    pandium: Pandium,
    deps: { gorgias?: GorgiasClient } = {}
): Promise<{ processed_events: Record<string, string> }> {
    const now = new Date()
